...
var config =
  require("../config/config.json")[mode] ||
...
function CermaiJs() {
  ...
  this.run = function() {
    ...
    config.app.port =
      process.env.OPENSHIFT_NODEJS_PORT || process.env.PORT || config.app.port;
    ...
    if (config.app.host == undefined) {
      ...
      app.listen(config.app.port, config.app.host, function() {
        console.log(
          "Application Running On " + config.app.host + ":" + config.app.port
        );
      });
    }
  };
  ...
  this.connect = function(cb) {
    if (config.connection == undefined) {
      ...
      if (config.connection.user == "" && config.connection.pwd == "") {
        mongoDB.connect(
          "mongodb://" + sideList + "/" + config.connection.db,
          function(err, db) {
            cb(err, db);
          }
        );
      ...
      if (
        (config.connection.user == "" || config.connection.user == undefined) &&
        (config.connection.pwd == "" || config.connection.pwd == undefined)
      ) {
        var mongouri =
